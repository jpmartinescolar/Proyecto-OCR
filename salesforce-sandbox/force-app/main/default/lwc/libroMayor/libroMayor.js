...
import getAdjacentCuenta from '@salesforce/apex/LibroMayorController.getAdjacentCuenta';
...
import exportarLibroMayorXlsx from '@salesforce/apex/LibroMayorExportController.exportarLibroMayorXlsx';
import updateMovimiento from '@salesforce/apex/PyGMensualController.updateMovimiento';

export default class LibroMayor extends NavigationMixin(LightningElement) {
    ...
    get textoInicial() {
        ...
        if (!this.cuentaId) f.push('una cuenta contable');
    ...
    refreshCuentas() {
        ...
        getCuentas({ empresasTitulares: this.empresasTitulares })
            .then(result => {
                ...
                // si la cuenta seleccionada ya no está disponible, la limpiamos
    ...
    handleGenerar() {
        ...
        if (!this.cuentaId) faltan.push('una cuenta contable');
    ...
    cargar(silent) {
        ...
        .then(result => {
            this.allRows = (result || []).map(m => this.mapRow(m));
            this.drillYm = null;
    ...
    mapRow(m) {
        ...
        return {
            // Clave estable por registro (no posicional): evita que LWC recicle
            // los lightning-record-picker de Director/Campaña entre filas distintas
            // y muestre valores fantasma al recargar.
            key: m.recordId,
            recordId: m.recordId,
    ...
    // ---------------------------------------------------------------
    handleAnterior()  { this.navegar('anterior'); }
    handleSiguiente() { this.navegar('siguiente'); }

    navegar(direccion) {
        if (!this.cuentaId || !this.empresasTitulares.length) return;
        this.navLoading = true;
        getAdjacentCuenta({
            cuentaId: this.cuentaId,
            direccion: direccion,
            ...
            extraordinarios: this.extraordinarios,
            check: this.check,
            acumuladoFiltro: this.acumuladoNav,
            incidencia: this.incidencia
        })
        .then(result => {
            if (!result) {
                ...
                this.dispatchEvent(new ShowToastEvent({
                    title: 'Sin más cuentas',
                    message: direccion === 'siguiente'
                        ? 'No hay una cuenta siguiente que cumpla los filtros.'
                        : 'No hay una cuenta anterior que cumpla los filtros.',
                    variant: 'info'
                }));
                return;
            }
            // si la cuenta no estaba en las opciones (p.ej. distinto orden), la añadimos
            if (!this.cuentaOptions.some(c => c.id === result.id)) {
                this.cuentaOptions = [...this.cuentaOptions, result];
            }
            this.cuentaId = result.id;
            this.cuentaNumero = result.numero;
            this.cuentaTitulo = result.titulo || '';
            this.cuentaApartado = result.apartado || '';
            this.cuentaSubapartado = result.subapartado || '';
            this.cargar(true);
        })
        .catch(err => {
            this.navLoading = false;
    ...
    // ---------------------------------------------------------------
    //  EXPORTAR A EXCEL (datos en pantalla + filtros aplicados)
    // ---------------------------------------------------------------
    exportarExcel() {
        const rows = this.rows || [];
        if (!rows.length) return;

        // Importes como número (Excel los suma); el resto, tal y como se ven
        const filas = rows.map(r => ({
            fecha: r.fecha,
            numeroApunte: r.numeroApunte,
            numeroAsiento: r.numeroAsiento,
            descripcion: r.descripcion,
            debe: this.unfmt(r.debe),
            haber: this.unfmt(r.haber),
            saldo: this.unfmt(r.saldo),
            empresaTitular: r.empresaTitular
        }));

        const cuentaNum = this.genCuentaNumero || '';
        const sufijo = `${(this.fechaDesde || '').replace(/-/g, '')}_${(this.fechaHasta || '').replace(/-/g, '')}`;

        // El .xlsx se genera en Apex (Compression.ZipWriter) y vuelve en base64
        exportarLibroMayorXlsx({
            empresa: (this.empresasTitulares || []).join(', ') || this.empresaLabel,
            desde: this.fmtFecha(this.fechaDesde),
            hasta: this.fmtFecha(this.fechaHasta),
            cuentaNumero: cuentaNum,
            cuentaTitulo: this.genCuentaTitulo || '',
            filasJson: JSON.stringify(filas)
        })
        .then(base64 => {
            this.descargarBase64(base64,
                `LibroMayor_${cuentaNum || 'cuenta'}_${sufijo}.xlsx`,
                'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        })
        .catch(err => {
            this.dispatchEvent(new ShowToastEvent({
                title: 'No se pudo exportar', message: this.reduceError(err), variant: 'error'
            }));
        });
    }

    // Descarga de contenido base64 vía data URI (compatible con Lightning Locker, donde blob: está bloqueado)
    descargarBase64(base64, nombre, mime) {
